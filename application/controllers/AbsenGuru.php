...
class AbsenGuru extends CI_Controller {
	...
	public function deleteData($id)
	{
		...
		$this->db->where('id', $id);
		$this->db->delete('tbl_absenguru');

		if ($this->db->affected_rows() === 1) {

			$url_image = FCPATH."asset/upload/files/".$file_excel;
            unlink($url_image);
			redirect('AbsenGuru','refresh');
		}
	...
	public function add()
	{
		$this->load->view('AbsenGuru/form_add');
	}

	public function index(){
		$data['absens'] = $this->get_data();
		$this->load->view('AbsenGuru/absenguru',$data);
	}
	...
	public function insert_data()
	{
		...
		if ( ! $this->upload->do_upload('dokumen')){
			...
			redirect('AbsenGuru/add','refresh');
		}
		else{
			...
			$this->db->insert('tbl_absenguru', $object);
